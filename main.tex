...
\author[2\authfn{1}*]{Matthew Cieslak}
\author[4]{Lei Ai}
\author[5]{Sendy Caffarra}
\author[4,5]{Alexandre R. Franco}
\author[6]{Iliana Karipidis}
\author[3]{John Kruper}
\author[4,5]{Michael Milham}
\author[6]{Barbara Avelar Pereira}
\author[6]{Ethan Roy}
\author[2]{Valerie J. Sydnor}
\author[6]{Jason Yeatman}
\author[7]{The Fibr Community Science Consortium}
% \input{fibr_authors.tex}
\author[2\authfn{2}]{Theodore D. Satterthwaite}
\author[3,1\authfn{2}]{Ariel Rokem}
...
\affil[5]{Nathan Kline Institute for Psychiatric Research, Center for Biomedical Imaging and Neuromodulation, Orangeburg, New York, 10962, USA}
...
\affil[7]{The Fibr Community Science Consortium}
...
\begin{abstract}
\todo[inline]{Comment ARF: I'm not a big fan of the word "derivatives" in the title. If you want to keep it, maybe place in another place? How about: "An open preprocessed and derivatives generated diffusion dataset for child and adolescent neuroimaging". Additionally, much of the work presented in the paper is regarding QC. Shouldn't that also be part of the title?  Please ignore if that doesn't make sense. }
We created resources to enable research on the role of brain connections in
development of mental health disorders, based on openly-available diffusion MRI
(dMRI) data from the Healthy Brain Network (HBN). We curated the
...
Data quality plays a key role in the analysis of these derivatives and relate them to development scores. To scale quality control (QC) to this large dataset, we
trained a convolutional neural network through combination of a small data
subset scored by experts and a larger set scored by web-based community
scientists. The network performs QC that highly agrees with expert QC on a left
...
structure and function \citep{Lebel2018-oy}. It is also a time during which the
...
imaging data in each individual. One of the brain imaging measurements acquired 
is diffusion MRI (dMRI), which is the dominant technology for describing the
...
\citep{avesani2019-ey}. \todo[inline]{ARF: Reference also this paper: Making data sharing work: The FCP/INDI experience, Mennes 2013, Neuroimage.} However, this raw, publicly available data requires extensive processing and quality assurance before it can be fruitfully analyzed.
...
data \citep{jones2010-ps}. Once preprocessing is completed correctly and
transparently, there is little need for researchers to repeat these steps. 
...
\citep{cieslak2021-iq}. To find the relevant files within a dataset, \emph{QSIPrep}
relies on the Brain Imaging Data Specification (BIDS) standard
\citep{gorgolewski2016-lh}, which describes an organizational structure of neuroimaging data
for automated processing. That is, it is a BIDS App \citep{Gorgolewski2017-mb}.
...
tasks, machine learning and computer vision methods, such as convolutional deep
artificial neural networks, or ``deep learning'' \citep{lecun2015deep} are
promising avenues for automation of QC. One of the challenges of these new
...
in that case, we deployed a community science application that collected quality
...
\mctodo[inline]{%
    Matt, please describe the \emph{QSIPrep} output directory structure.  Maybe
    we include a \texttt{tree} output figure and then a description of each
    output file.  Hint: Use
    \href{https://github.com/emad-elsaid/dirtree}{dirtree} to make a nice figure
    of \texttt{tree} output. Direct readers to FCP-INDI for both the
    BIDS-curated data and the \texttt{qsiprep} derivative dataset. Can you also
    please describe the three \emph{QSIPrep} metrics plotted in
    Figure~\ref{fig:metric-dist}, and write some text to refer to that figure?
}
...
(Figure~\ref{fig:fibr-qc:scatter:fibr}). In addition, different community scientists
...
we trained a gradient boosting model \citep{chen2016-eb} to predict expert scores
...
tractometry \citep{yeatman2012-rc}, graph theoretical analysis \citep{yeh2020-nu},
...
\citep{rudin2019stop}. The decision to use a more interpretable but slightly less
...
more extensive QC for each subject and more superficial QC on more subjects was
...
\citep{zunair2020-bs} previously applied to the assess tuberculosis severity
...
with the exception of the Fibr Community Science Consortium, whose members
