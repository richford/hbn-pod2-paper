...
\begin{abstract}
(For Scientific data max 170 words)

The Healthy Brain Network (HBN) is a landmark pediatric mental health study. Among other data, the HBN collects diffusion MRI (dMRI) from its participants, used to make inferences about brain connections. We curated the currently-available dMRI data from \num{2747} HBN subjects into the widely accepted Brain Imaging Data Structure (BIDS) format, we processed the data through a series of standard steps, to denoise the data and correct it for motion and eddy current effects. To perform quality control (QC) on this large dataset, we deployed a community science app. Together with a small subset of expert QC data, community science QC scores provided the information needed to train an artificial neural network that scores the data for quality in a manner that highly agrees with expert QC. We demonstrate that the neural network uses sensible features of the data and also demonstrate that accurate quality control has implications for subsequent analysis of this data. We openly released the processed derivatives and QC scores for use by the scientific community.
...
\cite{cieslak2021-iq}. To find the relevant files within a dataset,
\emph{QSIPrep} relies on the Brain Imaging Data Specification standard
\cite{gorgolewski2016}, which describes a way to organize neuroimaging data for
automated processing. That is, it is a BIDS App \cite{Gorgolewski2017-mb}. To
reach full compliance with the BIDS standard, data in HBN had to first be
curated \mctodo{maybe we need a couple more words here to say what that means?}.
...
generated by the deep learning algorithm are the HBN Preprocessed Open Diffusion
Derivatives (HBN-POD2). These derivatives provide researchers with everything
that they need in order to start applying a large variety of analysis
approaches, and should accelerate answering the important questions that this
data was collected to address.
...
BIDS-compliant dMRI data (Figure~\ref{fig:hbn-sankey}). These data are all accessible through the Amazon Web Services Open Data Program at \url{s3://fcp-indi/data/Projects/HBN/BIDS_curated/derivatives/qsiprep/}.
...
success of a previous application in assessing the quality of HBN's T1-weighted
MRI data \cite{keshavan2019-er}.
...
HBN-POD2 data using extensive visual examination of each subject's dMRI data,
including the dMRI time series itself, a plot of motion parameters throughout
the dMRI scan, and full 3D volumes depicting
...
\begin{enumerate*}[%
    ...
    itemjoin={{, }},%
    itemjoin*={{ and }}]
    \item the brainmask- and $b=0$-to-T1w registration
    \item a diffusion tensor imaging (DTI) directionally-encoded fractional anisotropy (FA) image overlayed on the $b=0$ volume.
\end{enumerate*}

\arhtodo{Add YouTube link}.

\arhtodo{Please add Pearson correlations between expert rating and QSIPrep QC metrics in the paragraph below}
...
and their human peers. This is not suprising, given that the XGB model was fit
to optimize this match.
...
neural network model training only on imaging data as CNN-i and the model that
...
provide information that is redundant with information available in imaging
...
that uses community science scores in addition to the \emph{QSIPrep} QC scores.
...
\begin{figure}[htbp]
    ...
    \caption{%
        ...
        information from community science ratings.
        %
...
scores by age (Figure \ref{fig:dl-qc:joint}), and sex and data collection site
...
shows attribution maps for example participants from each confusion class: true
positive, true negative, false positive, and false negative. The columns
correspond to the different channels of the deep learning input volume: the
$b=0$ reference image and the DEC-FA in the $x$, $y$, and $z$ directions. The
true positive map indicates that the network is looking at the entire brain
rather than focusing on any one anatomical region (Figure
...
\begin{figure}[tbp]
    ...
    \caption{%
        ...
        the DEC-FA images in the $x$, $y$, and $z$ directions. The first three columns
        ...
        focuses primarily on the B=0 channel, suggesting that it ignores DEC-FA
...
The effect of QC score on white matter tract profiles suggests that researchers
...
starting from the potential impact of the dataset itself and ending with
implications for curation, preprocessing and quality control of other
...
dataset, openly accessible to the public. In the past decade, data collection
projects such as the HBN, Human Connectome Project (HCP)
\cite{van-essen2013-oi}, UK Biobank \cite{miller2016-mq}, ABCD
\cite{jernigan2018-my}, CamCAN \cite{taylor2017-or,shafto2014-ld} and of
course FCP-INDI \cite{Mennes2013-eu} have ushered a culture of data sharing in
open big-data neuroscience. The adoption and reuse of these datasets reduces or
eliminates the data collection burden on downstream researchers.  Some data
collection projects, such as the HCP \cite{glasser2013-lo}, provide preprocessed
diffusion derivatives, further reducing researchers' burden and extending the
benefits of data-sharing from data collection to preprocessing and secondary
analysis. Following the example of the HCP, HBN-POD2 provides analysis-ready
dMRI derivatives. This avoids duplication of preprocessing effort while also
ensuring a minimum standard of data quality for HBN researchers. The data is
amenable to many different analyses, including tractometry \cite{yeatman2012-rc,
kruper2021evaluating}, graph theoretical analysis \cite{yeh2020-nu}, and
combinations with functional data for the same subjects. The availability of
standardized preprocessed diffusion data will allow researchers to create and
test hypotheses on the white matter properties underlying behavior and disease,
from reading and math acquisition to childhood adversity and mental health. As
such, this dataset will accelerate discovery at the nexus of structural
connectivity and neurodevelopmental and learning disorders.

\arhtodo{This paper looks relevant: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8111663/. We should take a closer look}
...
Importantly, the deep learning network architecture and parameters used to
determine the final QC scores are also provided as part of this work, allowing
application of this network to future releases of HBN data, and allowing other
researchers to build upon and improve upon this work. Moreover, undertaking this
processing and QC effort required construction and deployment of substantial
informatics infrastructure, including tools for cloud computing, web
applications for expert annotation and for community science rating and analysis
software. All of these tools are provided openly, so that this approach can be
adopted widely in other projects and in other scientific fields.
...
One of the challenges that arise from this work is the possibility of future improvements to processing pipelines and the release of future versions of processed data. It is currently an open question how to address such improvements, but one approach taken by some projects is to fix a "long-term support" version of a processing pipeline and to stick to this particular version for the duration of an extensive data collection effort.
...
\cite{nipype1,nipype2}, RRID:SCR\_002502. \emph{QSIPrep} is a robust and
scalable pipeline to group, distortion correct, motion correct, denoise,
coregister and resample MRI scans.  In total, \num{417} subjects failed this
preprocessing step, largely due to missing dMRI files. In keeping with the BIDS
specification, the preprocessed dataset is available as a derivative dataset
within the BIDS-curated dataset and can be access on AWS S3 at
...
\arhtodo{Please add the YouTube link in the paragraph below}
...
a tutorial video (which is available in the OSF project and also at XXX). They then rated each
...
\begin{figure}[tbp]
    ...
    \caption{%
        ...
        \arhtodo{This seems in contrast to the text below that suggests that the network predicts a continous QC score, not a probability of a binary score, which is what this text suggests}
        %
...
probability of passing QC. This architecture has \num{1438783} trainable
parameters.
...
\texttt{SimpleImputer} class. Because the HBN-POD2 bundle profiles exhibit
strong site effects \cite{richie-halford2021multidimensional}, we used the
ComBat harmonization method to robustly adjust for site effects in the tract
profiles. Initially designed to correct for site effects in gene expression
studies \cite{Johnson2007-kl}, ComBat employs a parametric empirical Bayes
approach to adjust for batch effects and has since been applied to multi-site
cortical thickness measurements \cite{fortin2018-hk}, multi-site DTI studies
\cite{fortin2017-be}, functional MRI data in the Adolescent Brain Cognitive
Development Study (ABCD) \cite{nielson2018detecting}. We rely on the
\emph{neurocombat\_sklearn} library \cite{neurocombat-sklearn}, to apply ComBat
...
different scanning sites as above. The XGBoost hyperparameters were hand-tuned
to values that have been performant in the authors' previous experience. We
log-transformed each participant's age before prediction using
...
or views of the Child Mind Institute. This work was supported via BRAIN Initiative grant 1RF1MH121868-01 from the National Institutes for Mental Health.
