...
(dMRI) data from the Healthy Brain Network (HBN). For this, we curated the
...
\arhtodo[inline]{First reference to S3 bucket hyperlink it.}
...
\mctodo[inline]{Please add a table of variant names, descriptions, and participant counts. Like the one you shared on Slack. Hint: use \texttt{df.to\_latex()}}

\arhtodo[inline]{Reference variant info in participants.tsv.}

\todo[inline]{In discussion, mention the variant annotation and allow end users to decide what they want to do.}
...
directions. 
\todo[inline]{Comment VJS: Because you describe this Figure in detail here, I'd add in what the blue voxels indicate (positive attribution/data supporting QC pass) and red voxels indicate (negative attribution/data supporting QC fail).}
...
\todo[inline]{Reword justification for the number of different scores. Maybe don't talk about reluctance and instead focus on generalization.}
...
\citep{rudin2019stop}. 
\todo[inline]{Same todo as above. Provide generalization as a possible motivation to use XGB-q over CNN-i.}
...
\mctodo[inline]{Even though variants were identified after preprocessing, I think we still need to explain where they came from. Can you briefly describe CuBIDS and the identification of the variants here (and cite the Zenodo). If you want, you can do as we did for the viewer and say that a full technical description of CuBIDS is forthcoming. But a brief introduction would be nice.}

\subsection{Preprocessing}

\mctodo[inline]{Matt: Please add information on edits to \emph{QSIPrep} to handle oblique acquisitions. There may be some redundancy with the QSIPrep paper but since the HBN data is what spurred that change, I think it's appropriate to mention here}
...
\todo[inline]{Comment VJS: May want to add a sentence explaining/justifying why DKI-derived measures were chosen here over other possible multishell modeling approaches + measures. Especially given that mrtrix was used for CSD and FOD reconstruction, so one may expect mrtrix's fiber density to be examined.}
...
hyruuk                     &                  0.000177 \\
\end{longtable}
\captionof{table}{Nonzero mean absolute shap values for the XGB model}
...
\end{longtable}
\captionof{table}{Mean absolute shap values for the XGB-q model}
