...
That is, it is a BIDS App \cite{Gorgolewski2017-mb}. To reach full compliance with the BIDS standard, data in HBN had to first be curated \mctodo{maybe we need a couple more words here to say what that means?}.
...
implications for quality control of other large-scale neuroimaging datasets.
...
In this kind of study, it is crucial to do accurate and reliable QC of the data.
We provide QC scores for the subjects that we have preprocessed, paving the way
for others to incorporate considerations of data quality into their analysis of
the processed data. Importantly, the deep learning network architecture for QC
...
build upon and improve upon this work. Moreover, undertaking this processing and
QC effort required construction and deployment of substantial informatics
infrastructure, including tools for cloud computing, web applications for expert
annotation and for community science rating and analysis software. All of these
tools are provided openly, so that this approach can be adopted widely in other
...
\arhtodo{We need the XGB-q QC scores in the derivatives on FCP-INDI}

We provided four different scores because we think that people might feel uncomfortable using the DL scores, despite the fact that we did the whole integrated gradients thing. XGB is great, but only for those subjects that have community scientist scores. A really decent fallback is XGB-q, and if you don't want to use the DL thing, maybe use that.
...
In this work, we have generalized our previous work on what we now call ``hybrid QC''. This approach, originally applied to the HBN T1-weighted data
\cite{keshavan2019-er} makes use of a small expert-labeled data set, a larger set of community scientist-labeled data and two steps of machine learning: the first to match between the community scientists and the experts, and the second to match between the images and the augmented community scientist labels.

This approach was generalized in several respects: first, from a single volume of T1-weighted data to the multi-volume dMRI.
