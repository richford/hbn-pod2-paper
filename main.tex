...
\arhtodo{cite studies/reviews that show the need for large sample sizes;
...
\arhtodo[inline]{This last sentence doesn't quite seem strong enough. Consider
rewording}. One of the common critiques of deep learning is that it can learn
irrelevant features of the data and does not provide information that is
transparent enough to interpret \arhtodo[inline]{cite relevant papers. the
radiology paper about the annotation of x-rays and so on}. Therefore, we used
machine learning interpretation methods in order to confirm whether the deep
learning algorithm uses meaningful features of the images to perform accurate QC
...
\arhtodo[inline]{Intro needs a lot more meat with relevant lit on importance of open datasets, dMRI, and QC.}
...
\arhtodo{Need to discuss comparison with previous T1-weighted work. One thing to discuss there is that expert inter-rater reliability was *a lot* higher for T1-weighted. This is potentially a problem, but also highlights how important it is to have an automated, "objective" approach.}
