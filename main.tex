...
currently-available HBN dMRI (N=\num{2747}) into the Brain Imaging Data
Structure (BIDS) and processed it acccording to established best-practices, to
denoise it and correct it for effects of motion, susceptibility-related
distortions, and eddy currents. Analysis of these data requires considerations
of varying data quality. To scale quality control (QC) to these large data, we
trained a convolutional neural network through combination of a small data
subset scored by experts, and a larger set scored through a web app by community
scientists. The network performs QC that highly agrees with expert QC on a left
out set. Taken together, the openly-available QC scores and processed data
provide a starting point for many analysis approaches.
...
\todo[inline]{%
    ...
    For eLife, the abstract should be around 150 words, which means we're over
    by a bit. Suggestions welcome.
...
symptoms of many mental health disorders tend to emerge \citep{Paus2008-gi}.
...
requires measurements from many individuals \citep{Paus2010-qk, Fair2021-eg}.
...
physical properties of developing white matter \citep{wandell2016-qt}. The dMRI
...
\citep{avesani2019-ey}. However, even with the data publicly available, several
...
data \citep{jones2010-ps}. Furthermore, once preprocessing is done correctly and
...
\citep{gorgolewski2016-lh}, which describes the way to organize neuroimaging data
...
\mctodo[inline]{%
    ...
    please discribe the three \emph{QSIPrep} metrics plotted in
...
\begin{fullwidth}
    ...
    \caption{%
        ...
        Scatterplots showing the relationship between mean expert rating and
...
\citep{taylor2017-or,shafto2014-ld} and of course FCP-INDI (which includeds HBN)
...
\alltodo[inline]{%
    We are targetting eLife for first submission and eLife now allows an
...
kurtosis imagin (DKI) model \citep{jensen2005-ta}, and impute missing bundles
