...
scientists. The network performs QC highly concordant with that of an expert on a left
out set.
...
Staten Island (SI, $N=300$), Rutgers University Brain Imaging Center (RU, $N=873$), the CitiGroup Cornell Brain Imaging Center (CBIC, $N=887$), and the City University of New York Advanced Science Research Center (CUNY, $N=74$), where the numbers in parentheses represent participant counts in HBN-POD2.
Site harmonization is a major issue for any multisite neuroimaging study and developing automated quality assessment tools that generalize between sites has been a perennial issue \cite{esteban2017mriqc}. Furthermore, the ability to generalize between sites in a single multisite study would signal the promise of generalizing to other datasets altogether. To better understand the ability of our QC models to generalize across scanning sites, we trained several variants of the XGB-q and CNN-i models on partitions of the data with different sites held out (Figure \ref{fig:site-generalization}). We find that ROC-AUC for generalization is uniformly high for both the XGB-q and the CNN-i models. However, more importantly, accuracy and balanced accuracy vary substantially: depending on the site that was used for training, balanced accuracy can be as low as guess rate, particularly for the CNN-i model. Notably, it seems that including the RU site in the training data leads to relatively high balanced accuracy in both models. The XGB-q model balanced accuracy seems less dependent on the specific sites used for training, but also displays some variability across permutations of this experiment. In particular, the benefit from including the ``right site'' in the training data, namely RU, eclipses the slight benefit conferred by including more than one site in the training data.
...
extension to the entire HBN-POD2 dataset and to future releases of HBN. However, it
...
(Figure~\ref{fig:ig}) that demonstrates the intuitive nature of its decisions. This also suggests that it might be able to generalize to similar images from other datasets. To assess this possibility, we trained the models with some sites held out. Both the CNN-i model and the XGB-q model can generalize to data from unseen sites. However, they don't always generalize and we cannot \emph{a priori} predict to which site they would generalize. This suggests that a procedure could be developed whereby the models that we developed here would be applied to data from other studies, but comprehensive calibration and validation would have to be undertaken. 
...
\end{enumerate*}
We view the XGB-q scores, which are available for all participants, as a more interpretable and scalable fallback because the XGB-q model ingests \emph{QSIPrep} output without any further postprocessing. XGB-q also provides slightly more uniform performance in generalization across HBN sites. Because the XGB-q model most readily generalizes to other \emph{QSIPrep} outputs, we package it as an independent QC service in the QSIQC software package \cite{richiehalford2022qsiqc}, available both as a docker image at \texttt{ghcr.io/richford/qsiqc} and as a Streamlit app at \url{https://share.streamlit.io/richford/qsiqc/main/app.py}.
...
We performed dMRI preprocessing on \num{2134} participants, using \emph{QSIPrep}
